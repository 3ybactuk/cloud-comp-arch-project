#!/usr/bin/env python3
# part 3b scheduler - runs parsec jobs in waves and measures makespan + slo
...
def get_memcached_addr(timeout_sec: int = 120) -> str:
    # node InternalIP + NodePort, same approach as run_part3a
    deadline = time.time() + timeout_sec
    while time.time() < deadline:
...
def parse_mcperf_samples(text: str) -> list[dict[str, float]]:
    ...
    for line in text.splitlines():
        ...
        try:
            ...
            qps = float(parts[16])
            ts_start = float(parts[-2]) / 1000.0
            ts_end = float(parts[-1]) / 1000.0
...
def get_policy() -> dict[str, Any]:
    ...
    # node-b-4core: 4 CPUs, all for batch
    #
    # from part2 runtimes (approx):
    #   freqmine ~250s, vips ~200s, canneal ~180s, streamcluster ~150s
    #   blackscholes ~60s, barnes ~50s, radix ~10s
    #
    # freqmine and vips thrash LLC so keep them off node-a if possible
    # canneal doesn't scale past ~2 threads anyway
    # best result so far: 378s (p07), bottleneck was freqmine+canneal sequential
    return {
        "memcached": {
            "node": "node-a-8core",
            "threads": 2,
