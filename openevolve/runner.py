...
MCPERF_REMOTE_DIR = "$HOME/memcache-perf-dynamic"
...
def start_mcperf(nodes: dict[str, dict[str, str]], mem_ip: str) -> None:
    # Kill any leftover mcperf from previous runs
    ...
    run_ssh(nodes["client-agent-a"]["name"],
            f"bash -c 'setsid {MCPERF_REMOTE_DIR}/mcperf -T 2 -A > /tmp/mcperf_agent_a.log 2>&1 </dev/null & disown'")
    run_ssh(nodes["client-agent-b"]["name"],
            f"bash -c 'setsid {MCPERF_REMOTE_DIR}/mcperf -T 4 -A > /tmp/mcperf_agent_b.log 2>&1 </dev/null & disown'")
    # Wait for agents to start listening
    time.sleep(10)
    ...
    run_ssh(nodes["client-measure"]["name"], f"rm -f {MCPERF_REMOTE_OUT}")
    run_ssh(nodes["client-measure"]["name"],
            (f"bash -c 'setsid {MCPERF_REMOTE_DIR}/mcperf -s {mem_ip}"
...
def fetch_mcperf_output(nodes: dict[str, dict[str, str]]) -> str:
    ...
    log(f"mcperf output: {len(data_lines)} data lines (total {len(all_lines)}), stderr: {res.stderr[:400] if res.stderr else 'none'}")
    if all_lines:
        log(f"mcperf first line: {all_lines[0][:200]}")
    return res.stdout
...
def parse_mcperf_samples(text: str) -> list[dict[str, float]]:
    ...
    for line in text.splitlines():
        ...
        if len(parts) < 17:
            continue
        try:
            p95 = float(parts[12]) / 1000.0  # p95 us -> ms
            qps = float(parts[16])
            ts_start = float(parts[-2]) / 1000.0  # ms -> s
            ts_end = float(parts[-1]) / 1000.0    # ms -> s
...
def compute_slo_metrics(samples: list[dict], batch_start: float, batch_end: float,
    ...
    # mcperf timestamps may be relative (not Unix), so skip time-window filter.
    # mcperf runs only during the batch, so all samples are in-window by construction.
    near_target = [s for s in samples if qps_low <= s["qps"] <= qps_high]
    if not near_target:
...
def run_policy(policy: dict[str, Any]) -> None:
    ...
    if all_ok:
        try:
            ...
            result["debug_mcperf_lines"] = len(mcperf_text.splitlines())
            result["debug_mcperf_samples"] = len(samples)
            result["debug_mcperf_first100"] = mcperf_text[:100]
