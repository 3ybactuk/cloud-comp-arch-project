...
\midrule
p01\_conservative\_isolation & Yes & 673 & --- &
  No mcperf samples captured in batch window (measurement timing issue;
  mcperf started before the service IP was stable). \\
...
makespan of 673\,s was already 37\% worse than p02 (489\,s) and therefore
...
This policy assigned \emph{all four LLC-heavy jobs} to node-b
(\texttt{blackscholes}, \texttt{canneal}, \texttt{freqmine},
\texttt{barnes}) and routed only \texttt{radix}, \texttt{streamcluster},
and \texttt{vips} to node-a — a stricter isolation strategy.
In its single screening run it recorded a makespan of \textbf{673\,s},
37\% longer than p02.
The root cause is that node-a became idle after \texttt{vips} finished at
${\approx}5$\,min while node-b was still processing \texttt{freqmine} and
\texttt{barnes} for another ${\approx}3.5$\,min.
